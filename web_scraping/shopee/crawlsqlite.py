...
output.to_sqlite(input_df=cats_parent, output_sqlite='database.db', table='categories')

# # Output SUB_category. Note, delete old file
cats_children = Cats.get_children()
output.to_sqlite(input_df=cats_children, output_sqlite='database.db', table='categories_sub')
...
def crawl_cat(path, page):
    ...
    output.to_sqlite(input_df=df_shops, output_sqlite='database.db',table="shops")
